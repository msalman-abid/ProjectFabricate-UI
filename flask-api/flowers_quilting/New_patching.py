...
def patch_img(image_path,num_block,retrieval=True,linewise=False,size=5):
    ...
    if linewise==True:
        ...
        for i in range(num_block):
            for j in range(num_block):
                ...
                if patch.shape[0]>=block_size or patch.shape[1]>=block_size:
                    ...
                    patch=resizeimage.resize_cover(patch, [block_size, block_size])
                    patch=patch.filter(ImageFilter.EDGE_ENHANCE_MORE)
                    patch=asarray(patch)
                else:
                    patch=cv2.resize(patch, (block_size,block_size),interpolation = cv2.INTER_AREA)
                    patch=Image.fromarray(patch)
                    patch=patch.filter(ImageFilter.EDGE_ENHANCE_MORE)
                    patch=asarray(patch)
        ...
        overlapping=[]
        leng=num_block+random.randint(5,8)
        ...
        for i in range(leng):
            ...
            # ar=np.argwhere(idx == False)
            # arr=list(map(tuple, ar))
            # hh = np.ones((h,p_w,3), dtype="uint8")
            # hh.fill(255)
            # temp=np.append(img,hh, axis = 1)
            # t_h,t_w,_=temp.shape
            # ww= np.ones((p_h,t_w,3), dtype="uint8")
            # ww.fill(255)
            # img=np.append(temp,ww, axis = 0)
            # h,w,_=img.shape
            ...
            if overlapping==[]:
                ...
                while exist==False:
                    ...
                    if num>=30:
                        ...
                        break 
                        # p_h, p_w, _ = patch.shape
                        # pat=Image.fromarray(patch_copy)
                        # # print(p_h,p_w)
                        # # print(patch_copy.shape)
                        # p_h=p_h-5
                        # p_w=p_w-5
                        # patch=resizeimage.resize_cover(pat, [p_h,p_w])
                        # patch=asarray(patch)
                        # idx = patch[:, :, 3] == 255
                    ...
                    if boxes==[]:
                        ...
                        coord_rect=[(i,j) for i in range(x,x+p_w) for j in range(y,y+p_h)]
                        for box in boxes:
                            coord_box=[(i,j) for i in range(box[0],box[2]) for j in range(box[1],box[3])]
    ...
    # new_size_h=block_size*(size-2)
    # new_size_w=block_size*(size)
    # new_im = np.ones((new_size_h,new_size_w,3), dtype="uint8")
    # new_im.fill(255)
...
# patch = cv2.imread('detected_objects/1sample7.png',cv2.IMREAD_UNCHANGED)

# patch_img('detected_objects/1sample7.png',10)

# cv2.imshow("Image", img)
# img = np.ones((500,500, 3), dtype="uint8")
# img.fill(255)
# patch_img(img,['detected_objects/0sample7.png','detected_objects/1sample7.png'],1000,retrieval=True)
